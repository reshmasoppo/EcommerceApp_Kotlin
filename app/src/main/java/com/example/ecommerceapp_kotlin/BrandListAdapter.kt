...
class BrandListAdapter (val Brand_Rec_list:ArrayList<BrandContent>): RecyclerView.Adapter<BrandListAdapter.ViewHolder>()
{
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int):BrandListAdapter.ViewHolder {

        val v= LayoutInflater.from(parent.context).inflate(R.layout.branditems_layout,parent,false)
        return ViewHolder(v)
    }
    override fun getItemCount(): Int {
        return Brand_Rec_list.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int)
    {
        val user3:BrandContent= Brand_Rec_list[position]
